#ifndef _DRAWLIGHTS_H_
#define _DRAWLIGHTS_H_

class CDrawLights{
private:
	static int iCTread;
	struct CDanoTread{int N;		int iStart;	/*int TreadNumber;*/};
...
public:
	//int fmMakeZBuffers;
	static int fnDrawZ(CCamera *pCam,CZLight *pZLight){
		CTrian3dbu mTr[64];
		//if(CGOption::check) EnterCriticalSection(&CGOption::cs);
		//if(CGOption::check){cout<<"START:"; system("pause");}
		double time;
		...
		for(;teksz<szVT;++teksz,++tekdegi,++trian){
			...
			{
				CTrian3dibu TrI;
				POINT hsz;	pZLight->fnGet_sz(hsz); hsz.x=hsz.x>>1; hsz.y=hsz.y>>1;
				for(int i=0;i<count;++i){
					//if(/*CGOption::check*/ mTr[i].v0.y<-10000){
					//	cout<<"   face "<<i<<":\n"; mTr[i].v0.fnPrint(); mTr[i].v1.fnPrint(); mTr[i].v2.fnPrint();
					//	cout<<"    Img:"<<mTr[i].Img<<'\n';
					...
					//	//system("pause");
					//}
					TrI.set(mTr[i],hsz);	pZLight->fnDrawTrian2D(TrI);		
				}
			}
			if(CGOption::checktime)	cout<<"fin:"<<(GetTickCount()-time )<<'\n';
		}
		//system("pause");
		//if(CGOption::check) LeaveCriticalSection(&CGOption::cs);
		return 0;
	}
};

int CDrawLights::iCTread=1;
